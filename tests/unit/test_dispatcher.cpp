...
void test_dispatch_creates_four_jobs() {
    ...
    [[maybe_unused]] uint64_t id = d.dispatch("What causes lightning?", "Context: thunderstorm clouds...");
    assert(id == 1);

    for (uint8_t i = 0; i < static_cast<uint8_t>(AgentRole::COUNT); ++i) {
        [[maybe_unused]] auto job = d.ring_for(static_cast<AgentRole>(i)).pop();
...
void test_job_ids_increment() {
    ...
    [[maybe_unused]] uint64_t id1 = d.dispatch("Query one", "ctx");
    [[maybe_unused]] uint64_t id2 = d.dispatch("Query two", "ctx");
    [[maybe_unused]] uint64_t id3 = d.dispatch("Query three", "ctx");
...
void test_context_truncation() {
    ...
    [[maybe_unused]] uint64_t id = d.dispatch("query", big_context);
    assert(id != 0);

    [[maybe_unused]] auto job = d.ring_for(AgentRole::Proposer).pop();
    assert(job.has_value());
    ...
    for (uint8_t role = 0; role < static_cast<uint8_t>(AgentRole::COUNT); ++role) {
        consumers.emplace_back([&, role]() {
            ...
            while (count < NUM_DISPATCHES) {
                [[maybe_unused]] auto job = d.ring_for(static_cast<AgentRole>(role)).pop();
